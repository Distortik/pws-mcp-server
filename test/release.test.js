...
test('release metadata agrees on the current stable version', function () {
    ...
    assert.match(server, /version: '0\.5\.0'/);
    assert.equal(packageJson.version.indexOf('-'), -1);
});

test('Workshop runtime manifest includes every required module', function () {
    ['index.js', 'mcpb-entry.js', 'mcp-server.js', 'plugin.json', 'THIRD_PARTY_NOTICES.txt', 'src/actions.js', 'src/audit.js', 'src/booking.js', 'src/bridge.js', 'src/domain.js', 'src/management.js', 'src/runtime.js', 'src/segments.js', 'src/tools.js'].forEach(function (file) {
